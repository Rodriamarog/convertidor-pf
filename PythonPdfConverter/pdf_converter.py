...
def main(input_path):
    ...
    try:
        with tempfile.NamedTemporaryFile(delete=False, suffix='.pdf') as flattened, \
            ...
            temp_files.extend([flattened.name, step1.name, step2.name])
            ...
            print("3. Removing blank pages...")
            remove_blank_pages(step1.name, step2.name)
            
            print("4. Converting to grayscale at 300 DPI with aggressive size optimization...")
            output_pdf = 'output.pdf'
            success = aggressive_compress(step2.name, output_pdf)
            
            if not success:
                print("\nWARNING: Could not reduce the PDF to under 3MB while preserving quality.")
                print("Consider these options:")
                print("1. Try manually removing unnecessary pages")
                print("2. Split the document into smaller parts")
                print("3. Try with a different PDF tool")
